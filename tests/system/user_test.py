...
class UserTest(BaseTest):
    def test_register_user(self):
        with self.app() as c: #client
            with self.app_context():
                #this test is pretending to be a client
                #data is being sent as FORM DATA!!
        ...
        with self.app() as c:
            with self.app_context():
                ...
                auth_request = c.post('/auth', data=json.dumps({
                    'username': 'test',
                    'password': '1234'
                }), headers={'Content-Type': 'application/json'}) #content type for json is jsonapplication
    ...
    def test_register_duplicate_user(self):
        with self.app() as c:
            with self.app_context():
                c.post('/register', data={'username': 'test', 'password': '1234'})
                r = c.post('/register', data={'username': 'test', 'password': '1234'}) #runs request second time
