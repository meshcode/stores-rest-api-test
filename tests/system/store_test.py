...
class StoreTest(BaseTest):
    ...
    def test_store_with_items_found(self):
        with self.app() as c:
            with self.app_context():
                StoreModel('test').save_to_db() #make a store
                ItemModel('test', 2.99, 1).save_to_db() #make an item for the store
                r = c.get('/store/test')

                self.assertEqual(r.status_code, 200)
                self.assertDictEqual(d1={'name': 'test', 'items': [{'name': 'test', 'price': 2.99}]},
                                     d2=json.loads(r.data)) #should be equal to respose data

    def test_delete_store(self):
        with self.app() as c:
            with self.app_context():
                StoreModel('test').save_to_db() #create store model
    ...
    def test_create_duplicate_store(self):
        with self.app() as c:
            with self.app_context():
                c.post('/store/test')
                r = c.post('/store/test') #creates second store
